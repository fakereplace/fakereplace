...
import org.fakereplace.com.google.common.collect.CustomConcurrentHashMap.ComputingStrategy;
import org.fakereplace.com.google.common.collect.CustomConcurrentHashMap.Internals;
import org.fakereplace.com.google.common.annotations.GwtCompatible;
import org.fakereplace.com.google.common.annotations.GwtIncompatible;
import org.fakereplace.com.google.common.base.FinalizableReferenceQueue;
import org.fakereplace.com.google.common.base.FinalizableSoftReference;
...
public final class MapMaker {
  ...
  private enum Strength {
    WEAK {
      ...
      @Override <K, V> ReferenceEntry<K, V> newEntry(
          CustomConcurrentHashMap.Internals<K, V, ReferenceEntry<K, V>> internals, K key,
  ...
  private static class StrongEntry<K, V> implements ReferenceEntry<K, V> {
    ...
    StrongEntry(CustomConcurrentHashMap.Internals<K, V, ReferenceEntry<K, V>> internals, K key,
        int hash) {
      this.internals = internals;
      this.key = key;
      this.hash = hash;
  ...
  private static class LinkedStrongEntry<K, V> extends StrongEntry<K, V> {
    ...
    LinkedStrongEntry(CustomConcurrentHashMap.Internals<K, V, ReferenceEntry<K, V>> internals,
        K key, int hash, ReferenceEntry<K, V> next) {
  ...
  private static class SoftEntry<K, V> extends FinalizableSoftReference<K>
    ...
    SoftEntry(CustomConcurrentHashMap.Internals<K, V, ReferenceEntry<K, V>> internals, K key,
        int hash) {
  ...
  private static class LinkedSoftEntry<K, V> extends SoftEntry<K, V> {
    LinkedSoftEntry(CustomConcurrentHashMap.Internals<K, V, ReferenceEntry<K, V>> internals,
        K key, int hash, ReferenceEntry<K, V> next) {
      super(internals, key, hash);
      this.next = next;
    }
  ...
  private static class WeakEntry<K, V> extends FinalizableWeakReference<K>
      implements ReferenceEntry<K, V> {
    WeakEntry(CustomConcurrentHashMap.Internals<K, V, ReferenceEntry<K, V>> internals, K key,
        int hash) {
      super(key, QueueHolder.queue);
      this.internals = internals;
      this.hash = hash;
  ...
  private static class LinkedWeakEntry<K, V> extends WeakEntry<K, V> {
    LinkedWeakEntry(CustomConcurrentHashMap.Internals<K, V, ReferenceEntry<K, V>> internals,
        K key, int hash, ReferenceEntry<K, V> next) {
      super(internals, key, hash);
      this.next = next;
    }
